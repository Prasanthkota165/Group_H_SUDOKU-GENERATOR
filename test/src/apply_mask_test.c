...
int main()
{
   ...
   int rearranged_sudoku1 [9][9] = {{1, 2, 3, 4, 5, 6, 7, 8, 9},
    ...
    fprintf(fp, "%s", "----------------------------------------------------------\n");
    fprintf(fp, "%s", "PRECONDITIONs are asserted at the function level, therefore only error messages will display here \n");
    fprintf(fp, "%s", "\n");
    ...
    /* apply_mask function being called with rearranged_sudoku array size 4x9 instead of 9x9 */
    ...
    apply_mask(rearranged_sudoku1, 7 , masked_sudoku);
