...
int main(void){
    int sudoku[9][9]; /* Refers to the input parameter for the 
...
_Bool swap_values(int sudoku[][9]){
    ...
    for(i=0;i<9;i++){
	    for(j=0;j<9;j++){
		    if(sudoku[i][j]!=a[i][j]){ /* check how many numbers in the row are
                                         swapped */ 
			    check++;		
		    }
		    fprintf(log,"  %d ", a[i][j]);
...
_Bool beyond_boundary_1(int sudoku[][9]){
    ...
    for(i=0;i<l;i++){
    	for(j=0;j<k;j++){ /* check rows whether it has any elements beyond the 
                            boundaries */
		    if((a[i][j]>9) || (a[i][j]<1)){
			    check_1++;		
		    }
    ...
    for(i=0;i<l;i++){ /* check columns whether it has any elements beyond the 
                        boundaries */
    	for(j=0;j<k;j++){
		    if((a[j][i]>9) || (a[j][i]<1)){
			    check_2++;		
...
_Bool beyond_range_2(int rearranged_sudoku[][9]){
    ...
    for(i=0;i<9;i++){
	    for(j=0;j<9;j++){/* checking columns for in [1,9] */
		    if((rearranged_sudoku[j][i] <=9) && (rearranged_sudoku[j][i]>=1)){
			    check_5++;			
		    } else {
			    check_5--;		
		    }
