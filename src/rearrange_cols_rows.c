 ...
 *
 * Purpose   : This function is used to generate different combinations of the 
 * 				sudoku, by randomly interchanging rows,columns of the Sudoku 
 * 				Template.
 *
 * Language  : C
 * 
...
/**
 * Four different numbers will be randomly selected from each row and swap 
 * between them.
 */ 
void swap(int table[][9]){
	...
	/* This condition is to make sure all the 4 numbers are different */
  		...
  		if((interchange_1 != interchange_2) && \
 			...
 			break;/* If the 3 numbers are not different,then generate 3 random 
			 		numbers again */
...
void change_columns(int col_matrix[][9], int p, int q, int r){
    ...
    int num_1,num_2,num_3; /*choose the columns to get swapped with random 
							generator*/
...
void change_rows(int row_matrix[][9], int x, int y, int z){
    int i, count_1,count_2,count_3;
    int ele_1,ele_2,ele_3;/*choose the rows to get swapped with random 
							generator*/
 ...
 */
void col_groups(int column_group[][9], int b_1[9][3], int b_2[9][3], 
					int b_3[9][3]){
    int i,j;
    int k,l;
    ...
    while(1){/* All the column groups should be different */
 		if((ch_1!=ch_2) && (ch_2!=ch_3) && (ch_3!=ch_1)){
  			for(i=0;i<9;i++){
				...
				for(j=0;j<3;j++){/* copy into the column_group(9x9)
					 to interchange column groups(9x3) */
					if (order == 1){
				...
				k=0;
				order = ch_2;/* copy second column group as ch_2 */
				for(j=3;j<6;j++){
 					if (order == 1){
				...
				l=0;
				order = ch_3;/* copy third column group as ch_3 */
 ...
 */
void row_groups(int r_group[][9], int c_1[3][9], int c_2[3][9], 
					int c_3[3][9]){
    int i,j;
    int k,l;
    ...
    while(1){/* All the row groups should be different */
 		if((rw_1!=rw_2) && (rw_2!=rw_3) && (rw_3!=rw_1)){
  			for(i=0;i<9;i++){
				...
				for(j=0;j<3;j++){ /* copy into the r_group(9x9)
									to interchange row groups(3x9) */
					if (order == 1){
 						r_group[j][i]=c_1[j][i];
					} else if (order == 2){
						r_group[j][i]=c_2[j][i];
				...
				k=0;
				order = rw_2;/* copy 2nd row group as rw_2 */
				for(j=3;j<6;j++){
 					if (order == 1){
 						r_group[j][i]=c_1[k][i];
					} else if (order == 2){
				...
				l=0;
				order = rw_3;/* copy 3rd row group as rw_3 */
				for(j=6;j<9;j++){
 					if (order == 1){
 						r_group[j][i]=c_1[l][i];
					} else if (order == 2){
