...
_Bool test_sudoku_rules(int rearranged_sudoku[][9]){  
    ...
    x= test_rows(rearranged_sudoku,a); /*Checks for any repeated values in the 
                                        rows*/
    y= test_cols(rearranged_sudoku,a); /*Checks for any repeated values in the 
                                          columns*/
    z= test_3X3(rearranged_sudoku,a); /*Checks for any repeated values in the 
                                        3X3 matrices*/
    if(x&&y||y&&z||z&&x){
  	    return true;
    }
    ...
    for (i=0;i<9;i++) {
    	  ...
    	  for(p=0;p<9;p++){
	 	        if(a[p]!=0 || sum!=45){ /* If the array is not empty or
			          ...
			          flag++;/*Increment flag with no. of times the sudoku rules 
                          failed*/
		        }
  	    }
   	    sum=0;
...
_Bool test_cols(int rearranged_sudoku[][9], int a[9]){
    ...
    for (i=0;i<9;i++) {
	      for (j=0;j<9;j++) {
		        check_2=rearranged_sudoku[j][i];/*Copy each element of column to 
                                              check*/
		        sum=sum+check_2; /* summing all the elements in the column to get 
                                45 */
		        delete_1(check_2,a);
	      }
    	  for(p=0;p<9;p++){
...
_Bool test_3X3(int rearranged_sudoku[][9], int a[9]) {  
    ...
    while(x<=8) {
        while(k<=8) {
            for (int i=x;i<=y;i++) {
                for(int j=k;j<=l;j++) {
   		              check_3=rearranged_sudoku[i][j];/*Copy each element of 3X3 
                                                      matrix to check*/
   		              sum=sum+check_3; /* summing all the elements in the 3X3 
                                        matrix to get 45 */
                    delete_1(check_3,a);
                }
            }
            ...
            l=l+3;
            for(p=0;p<9;p++){
	 	            if(a[p]!=0 || sum!=45){ /* If the array is not empty or sum is 
                                          not 45 then test failed*/
			              flag++;/*Increment flag with no. of times the sudoku rules 
                            failed*/
		            }
            }  
...
/* Mask the unrepeated elements in row/column/3x3 block and leave the array with 
 * elements that has to be appeared in the place of repeated values.
 */
